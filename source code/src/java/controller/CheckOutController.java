...
public class CheckOutController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        for (Cart cart : listCartSession) {
            amount += cart.getQuantity();
        }
        Users u = (Users) session.getAttribute("usersession");

        ProductOrderDAO prDao = new ProductOrderDAO();
        OrderDetailDAO orDao = new OrderDetailDAO();

        prDao.addOrder(u.getUserId(), amount);

        for (Cart cart : listCartSession) {
            orDao.addOrderDetail(Integer.parseInt(prDao.getLatestId()), cart.getProductId(),
